...
module.exports = function(reader) {
    class Client extends EventEmitter {
        ...
        subscribe(chanel, cb) {
            checkType(chanel, 'string', 'Chanel name');
            checkType(cb, 'function', 'Subscribe listener');
            if(!this[P_SUBSCRIBERS][chanel]) {
                this[P_SUBSCRIBERS][chanel] = [cb];
                this[M_SEND]({
                    t: T_SUBSCRIBE,
                    c: chanel
                });
            ...
            this[P_SUBSCRIBERS][chanel].push(cb);
        }

        unsubscribe(chanel, cb) {
            const subscribers = this[P_SUBSCRIBERS][chanel];
            if(!subscribers) { return; }
            if(!cb || (subscribers.length === 1 && subscribers[0] === cb)) {
                delete this[P_SUBSCRIBERS][chanel];
                this[M_SEND]({
                    t: T_UNSUBSCRIBE,
                    c: chanel
                });
